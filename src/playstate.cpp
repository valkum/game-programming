...
void PlayState::Init(CGame* game) {
    ...
    // debugShader = ShaderProgramCreator("debug").attributeLocations(debug_vao->getAttributeLocations()).create();
...
void PlayState::Draw(CGame* g, float* delta) {
    ...
    glm::mat4 viewProjectioMatrix = camera.getProjectionMatrix() * camera.getViewMatrix();
...
void PlayState::HandleKeyEvents(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if(action == GLFW_PRESS | action == GLFW_REPEAT) {
        ...
        if(key==GLFW_KEY_P) {
            debug()<<"uViewProjectionMatrix Location = "<<glGetUniformLocation(cubeShader->getObjectName(), "uViewProjectionMatrix")<<endl;
            debug()<<"uViewMatrix Location = "<<glGetUniformLocation(cubeShader->getObjectName(), "uViewMatrix")<<endl;
            debug()<<"uMVP Location = "<<glGetUniformLocation(cubeShader->getObjectName(), "uMVP")<<endl;
            debug()<<"uProjectionMatrix Location = "<<glGetUniformLocation(cubeShader->getObjectName(), "uProjectionMatrix")<<endl;
        }
